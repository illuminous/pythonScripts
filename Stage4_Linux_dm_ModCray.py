...
def createInFiles(zone, path0, path1, path2, path4):
    for chunk in res:
        myfile = path2 + zone+'_'+ '%s' %(chunk)+'.in'
...
def createBashFile(zone, path0, path2):
    myfile = path2 + zone + '_' + 'fireharm' + '.bash'
    print myfile
    infile = open(myfile, 'w') 
...
def createBATS(zone, path0, path2, path3):
    for chunk in res:
        batfile = path2 + '%s_%s' %(zone, chunk) + '.bat'
        print batfile
...
def main():
    ...
    for z in zones:
        try:
            ...
            path4 = 'state/partition1/'
            chunkRange()
            createInFiles(z, path0, path1, path2, path4)
