...
query="""SELECT classified_FLM_single_condition.EVT_value, classified_FLM_single_condition.EVC_Cover10, classified_FLM_single_condition.EVH_Breaks, classified_FLM_single_condition.FLM, Count(classified_FLM_single_condition.FLM) AS CountOfFLM
FROM classified_FLM_single_condition
GROUP BY classified_FLM_single_condition.EVT_value, classified_FLM_single_condition.EVC_Cover10, classified_FLM_single_condition.EVH_Breaks, classified_FLM_single_condition.FLM;
...
while not result.EOF: #iterate over the records, one row at a time
    ...
    col3=result.Fields.Item(2).Value #maybe EVH?
...
#open an outfile
outfile = open("C:/temp/evt_evc_evh.csv",'w') 

#write a header - turn this off if reading into Arc Info table
outfile.write("EVT,EVC,EVH,FLM,Count\n")

for col1 in mydata: #for every BPS
    for col2 in mydata[col1]: #for every EVT in this BPS
        for col3 in mydata[col1][col2]:#for every EVC
            temp_list=[] #temporary list for sor
            for col4 in mydata[col1][col2][col3]: #for every FLM in this BPS/EVT/EVC combo
