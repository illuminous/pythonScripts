...
zone = 'z16'
root = 'L:/event_fireharm/'
prep = 'L:/event_prep/'
emds = ''
infiles = '/home/jherynk/' + zone + '/infiles/'
outfiles = '/mnt/pvfs2/jherynk/' + zone + '/outfiles/'
batLoc = '/home/jherynk/' + zone
mode = 'dm'
workingdir =   infiles
...
def createBashFile():
    ...
    infile.write('datadir=/home/jherynk/' + zone + '\n')
    infile.write('cd ${datadir}\n')
    directions = """
qsub -cwd -S /bin/bash ./%s_1.bat
qsub -cwd -S /bin/bash ./%s_2.bat
qsub -cwd -S /bin/bash ./%s_3.bat
qsub -cwd -S /bin/bash ./%s_4.bat
qsub -cwd -S /bin/bash ./%s_5.bat
qsub -cwd -S /bin/bash ./%s_6.bat
qsub -cwd -S /bin/bash ./%s_7.bat
qsub -cwd -S /bin/bash ./%s_8.bat
qsub -cwd -S /bin/bash ./%s_9.bat
...
qsub -cwd -S /bin/bash ./%s_16.bat
qsub -cwd -S /bin/bash ./%s_17.bat
qsub -cwd -S /bin/bash ./%s_18.bat
qsub -cwd -S /bin/bash ./%s_19.bat
qsub -cwd -S /bin/bash ./%s_20.bat
...
def createBATS():
    for chunk in res:
        ...
        infile.write(batLoc + '/' + 'fireharm' + ' ')
        infile.write(batLoc + '/' + zone + '_' + '%s' %(chunk) + '.in')
        infile.close()
                     
def copyASCII():
    for chunk in res:
        filename1 = prep +'/' + zone +'/'+zone + 'poly' + '%s' %(chunk)+'.asc'
        print filename1
