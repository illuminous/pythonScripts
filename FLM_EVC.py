...
query="""SELECT classified_FLM_single_condition.EVT_value, classified_FLM_single_condition.EVC_Cover10, classified_FLM_single_condition.FLM, Count(classified_FLM_single_condition.FLM) AS CountOfFLM
FROM classified_FLM_single_condition
GROUP BY classified_FLM_single_condition.EVT_value, classified_FLM_single_condition.EVC_Cover10, classified_FLM_single_condition.FLM;
...
while not result.EOF: #iterate over the records, one row at a time
    ...
    col1=result.Fields.Item(0).Value  #maybe EVT ungrouped?
    col2=result.Fields.Item(1).Value #maybe EVC?
    col3=result.Fields.Item(2).Value #maybe FLM?
    col4=result.Fields.Item(3).Value #maybe FLM Count?
    ...
    if col1 and col2 and col3 and col4:#make sure these are all valid
        ...
        mydata[col1][col2][col3]=col4    #this is a dictionary that stores BPS/EVT combinations, and stores the count for each combination
...
#open an outfile
outfile = open("C:/temp/evt_evc.csv",'w') 

#write a header - turn this off if reading into Arc Info table
outfile.write("EVT,EVC,FLM,Count\n")

for col1 in mydata: #for every EVT
    for col2 in mydata[col1]: #for every Evt in this sc
            temp_list=[] #temporary list for sor
            for col3 in mydata[col1][col2]: #for every BPS in this BPS/EVT combo
                temp_list.append([col3,mydata[col1][col2][col3]]) #store a pair of values - the first is the FLM, the second is the count
            ...
            for i in range(1,len(temp_list)): #compare cur_count to remaining counts.
                ...
                if highest_count==cur_count:
                    ...
                    break
            if majority:
                outfile.write("%s,%s,%s,%s\n"%(col1,col2,highest_FLM,highest_count))
