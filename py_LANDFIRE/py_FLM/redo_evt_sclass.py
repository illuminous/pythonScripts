...
query="""SELECT tbl_Products.VEG_evt, tbl_Products.FR_sclass, tbl_fire_effects_FLM.FLM, Count(tbl_fire_effects_FLM.FLM) AS CountOfFLM
FROM tbl_fire_effects_FLM INNER JOIN tbl_Products ON tbl_fire_effects_FLM.Master_ID = tbl_Products.Master_ID
GROUP BY tbl_Products.VEG_evt, tbl_Products.FR_sclass, tbl_fire_effects_FLM.FLM
ORDER BY tbl_Products.VEG_evt;""" #paste SQL query created in Access here
...
while not result.EOF: #iterate over the records, one row at a time
    ...
    col2=result.Fields.Item(1).Value #maybe FR_Sclass?
    ...
    if col1 and col2 and col3 and col4:#make sure these are all valid
        ...
        mydata[col1][col2][col3]=col4    #this is a dictionary that stores ESP/EVT/Fr_Sclass/FLM combinations, and stores the count for each combination
...
#open an outfile
outfile = open("C:/AWORKSPACE/flm_workspace/flm_production/Python_Majority_Outfiles/evt_sclass.csv",'w') 

#write a header - turn this off if reading into Arc Info table
outfile.write("EVT,FR_Sclass,FLM,Count\n")

for col1 in mydata: #for every EVT
    for col2 in mydata[col1]: #for every Evt in this sc
            temp_list=[] #temporary list for sor
            for col3 in mydata[col1][col2]: #for every FLM in this ESP/EVT/sclass combo
